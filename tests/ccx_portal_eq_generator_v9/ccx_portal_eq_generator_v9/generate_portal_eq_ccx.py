...
Why this exists:
- Rotational SPRING2 on DOF 4..6 is tricky in CalculiX because most analyses use 3 DOF per node.
- Rigid-body ROT nodes are a reliable way to get "rotation about axis" represented as U1/U2/U3 (rad),
  and translational springs on those DOFs behave as rotational springs (moment vs rotation).

This generator builds:
- Portal frame (2 columns + top beam) as C3D8R hexa blocks (structured mesh, positive Jacobians)
- Rigid bodies at 4 joint faces (COL_L_TOP, BEM_L, COL_R_TOP, BEM_R) with REF+ROT nodes
- Translation continuity at each joint via *EQUATION (U1..U3 of REF nodes)
- Hinges as SPRING2 between ROT nodes on DOF=3 (theta_z) with NONLINEAR M-θ table
- STEP 1: Static gravity (optional)
- STEP 2: Implicit dynamic earthquake body acceleration as DLOAD GRAV in +X with AMPLITUDE=EQX

Energy output:
- CalculiX does NOT support "*ENERGY PRINT" (Abaqus keyword).
- For nonlinear dynamics, request energy from the FIRST step using *EL PRINT ... ENER.
"""
...
def write_inp(out_inp: Path, t: List[float], ax: List[float], *,
              L: float, H: float, b: float, h: float, tz: float,
              nL: int, nH: int, nx: int, nz: int,
              E: float, nu: float, rho: float,
              hinge_my: float, hinge_k_el: float, hinge_k_post: float,
              dt0: Optional[float], gravity: bool, keep_gravity_in_dynamic: bool) -> Dict:
    nid, eid = 1, 1

    # Columns: width b in X, height H in Y, thickness tz in Z
    colL, nid, eid = build_block(nid, eid, (0.0,0.0,0.0), (b,0,0), (0,H,0), (0,0,tz), nx, nH+1, nz)
    colR, nid, eid = build_block(nid, eid, (L-b,0.0,0.0), (b,0,0), (0,H,0), (0,0,tz), nx, nH+1, nz)

    # Beam: spans full L in X, depth h in Y, thickness tz in Z, located at top: y in [H-h, H]
    beam, nid, eid = build_block(nid, eid, (0.0, H-h, 0.0), (L,0,0), (0,h,0), (0,0,tz), nL+1, nx, nz)
    ...
    # REF/ROT nodes at 4 joint faces
    ...
    # SPRING2 elements between ROT nodes
    EID_HL = max_e + 1
    EID_HR = max_e + 2
    ...
    # centroids for ref/rot placement
    ...
    with out_inp.open("w") as f:
        f.write("** portal_eq.inp generated by ccx_portal_eq_generator v9\n")
        f.write("*HEADING\nPortal EQ (solids+hinges)\n\n")
        f.write("*NODE\n")
        ...
        # Translational continuity at joints (leave rotations free via hinge spring)
        for d in (1,2,3):
            f.write("*EQUATION\n2\n")
        ...
        # Hinge springs about z (theta_z is U3 at ROT nodes in rigid body formulation)
        ...
        # Optional gravity step
        if gravity:
            f.write("*STEP\n*STATIC\n")
            f.write("*DLOAD\n")
            f.write("E_ALL, GRAV, 9.81, 0., -1., 0.\n")
            # Request energy from FIRST step (nonlinear dynamics rule)
            f.write("*EL PRINT, ELSET=E_ALL\n")
            f.write("ENER\n")
            f.write("*END STEP\n\n")

        # Dynamic step: earthquake as body acceleration in X via GRAV with amplitude
        f.write("*STEP\n*DYNAMIC\n")
        f.write(f"{dt0:.8e}, {T:.8e}\n")
        if keep_gravity_in_dynamic:
            f.write("*DLOAD\n")
            f.write("E_ALL, GRAV, 9.81, 0., -1., 0.\n")
    ...
    hinge_map = {
        "rot_nodes": {"ROT_L_COL": ROT_L_COL, "ROT_L_BEM": ROT_L_BEM, "ROT_R_COL": ROT_R_COL, "ROT_R_BEM": ROT_R_BEM},
        "hinges": [{"name":"H_L","nodeA":ROT_L_COL,"nodeB":ROT_L_BEM},{"name":"H_R","nodeA":ROT_R_COL,"nodeB":ROT_R_BEM}],
        "note": "Rigid-body ROT nodes: U1/U2/U3 are rotations about global X/Y/Z in radians.",
    }
    return hinge_map
...
def main() -> None:
    ...
    ap.add_argument("--tz", type=float, default=0.20)
    ...
    ap.add_argument("--hinge_my", type=float, default=2.0e5)      # N*m
    ap.add_argument("--hinge_k_el", type=float, default=1.0e8)    # N*m/rad
    ap.add_argument("--hinge_k_post", type=float, default=1.0e6)  # N*m/rad
    ap.add_argument("--dt0", type=float, default=None)
    ap.add_argument("--gravity", action="store_true")
    ap.add_argument("--keep-gravity-in-dynamic", action="store_true")
    args = ap.parse_args()

    t, ax = read_eq_csv(args.eq)
    hinge_map = write_inp(args.out, t, ax,
                         L=args.L, H=args.H, b=args.b, h=args.h, tz=args.tz,
                         ...
                         dt0=args.dt0, gravity=args.gravity, keep_gravity_in_dynamic=args.keep_gravity_in_dynamic)
    print(f"Wrote {args.out}")
    print(f"Solid nodes: {max(hinge_map['rot_nodes'].values())-8} (REF/ROT nodes appended: {max(hinge_map['rot_nodes'].values())-7}..{max(hinge_map['rot_nodes'].values())})")
    print("Hinge map written to hinge_map.json")
    Path("hinge_map.json").write_text(json.dumps(hinge_map, indent=2), encoding="utf-8")
