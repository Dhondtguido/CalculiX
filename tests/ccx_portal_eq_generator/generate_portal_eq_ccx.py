#!/usr/bin/env python3
"""
Generate a CalculiX portal frame with two rotational (M-θ) hinges and a single horizontal earthquake record.

Modeling approach (robust in CalculiX):
- Solid members (C3D8R) for columns + beam, with enough elements along the member to flex like a beam.
- At each beam-column joint, create two rigid bodies (one for column top face, one for beam end face).
- Tie their reference-node translations with *EQUATION (U1/U2/U3).
- Connect their rotational nodes with a SPRING2 acting on DOF=3 (rotation about global Z).
  In CalculiX rigid bodies, the ROT-node "U1/U2/U3" represent rotations about global X/Y/Z (radians).

Earthquake excitation:
- Uses a time-dependent body acceleration via *DLOAD, GRAV, AMPLITUDE=EQX
  (reference magnitude = 1.0, amplitude values in m/s^2).

Usage:
  python3 generate_portal_eq_ccx.py --eq earthquake.csv --out portal_eq.inp

The earthquake CSV must have columns:
  t, ax_mps2   (time in seconds, acceleration in m/s^2)
...
@dataclass
class Block:
    nodes: Dict[Tuple[int, int, int], int]  # (i,j,k)->nid
    node_coords: Dict[int, Vec]
    elements: List[Tuple[int, Tuple[int,int,int,int,int,int,int,int]]]
    face_nodes: Dict[str, List[int]]  # i0,i1,j0,j1,k0,k1
...
def write_inp(out_inp: Path, t: List[float], ax: List[float], *,
    ...
    with out_inp.open("w") as f:
        f.write("** Portal frame (solids) + M-theta hinges, earthquake via *DLOAD GRAV AMPLITUDE\n")
        f.write("*HEADING\nPortal EQ test\n\n")
        f.write("*NODE\n")
        for blk in (colL, colR, beam):
    ...
    print(f"Nodes solids={len(colL.node_coords)+len(colR.node_coords)+len(beam.node_coords)} + 8")
    print(f"Elems solids={len(E_ALL)} + 2 springs")
    print(f"Hinge theta_y={th_y:.3e} rad (My={hinge_my:.3e} N*m)")
