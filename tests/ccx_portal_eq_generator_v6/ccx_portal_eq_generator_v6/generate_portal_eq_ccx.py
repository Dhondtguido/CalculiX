...
- STEP 1: gravity (*STATIC)
- STEP 2: base-acc-like horizontal excitation via *DLOAD GRAV, AMPLITUDE=EQX

Hinges:
- *RIGID BODY with REF/ROT nodes at joints (top of columns and ends of beam)
- translations tied via *EQUATION; rotation about z provided by nonlinear springs between ROT nodes
- ROT node U3 is interpreted as theta_z (rad) in rigid-body formulation

CCX quirk:
Some builds require *ENERGY PRINT to be present in STEP 1 when doing nonlinear dynamics in STEP 2.
So we ALWAYS emit *ENERGY PRINT in STEP 1 (when gravity is on) and STEP 2.
"""
import argparse, csv
...
def energy_print(f) -> None:
    f.write("*ENERGY PRINT\n")
    f.write("ALLIE, ALLKE, ALLSE, ALLVD, ALLPD, ETOTAL\n")
...
def write_inp(out_inp: Path, t: List[float], ax: List[float], *,
    ...
    nid, eid = 1, 1
    # Columns: (width b) x (height H) x (thickness t_z)
    ...
    # REF/ROT nodes ABOVE max solid node id
    ...
    with out_inp.open("w") as f:
        f.write("** portal_eq.inp generated by v6\n")
        f.write("*HEADING\nPortal EQ (v6)\n\n")
        f.write("*NODE\n")
