...
!
      subroutine shape5p(xi,et,ze,xl,xsj,shp,iflag)
!
!     shape functions and derivatives for a 5-node linear
...
!     The shape functions is based on the 5-node Bergot Pyramid
!     described by 
!       Morgane Bergot
!       HIGHER-ORDER FINITE ELEMENTS FOR HYBRID MESHES
!         USING NEW NODAL PYRAMIDAL ELEMENTS
!       https://doi.org/10.1007/s10915-009-9334-9
!
!
      implicit none
!
      integer i,j,k,iflag
!
      real*8 shp(4,5),xs(3,3),xsi(3,3),xl(3,5),sh(3)
!
      real*8 xi,et,ze,xsj
!
!     shape functions
!
      shp(4,1) = 0.25d0*(1.d0-xi-et-ze+xi*et/(1.d0-ze))
      shp(4,2) = 0.25d0*(1.d0+xi-et-ze-xi*et/(1.d0-ze))
      shp(4,3) = 0.25d0*(1.d0+xi+et-ze+xi*et/(1.d0-ze))
      shp(4,4) = 0.25d0*(1.d0-xi+et-ze-xi*et/(1.d0-ze))
      shp(4,5) = ze
!
...
!
!
      shp(1,1) = 0.25d0*(et/(1.d0-ze)-1.d0)
      shp(1,2) = 0.25d0*(-et/(1.d0-ze)+1.d0)
      shp(1,3) = 0.25d0*(et/(1.d0-ze)+1.d0)
      shp(1,4) = 0.25d0*(-et/(1.d0-ze)-1.d0)
      shp(1,5) = 0.d0
!
!     local derivatives of the shape functions: eta-derivative
!
      shp(2,1) = 0.25d0*(xi/(1.d0-ze)-1.d0)
      shp(2,2) = 0.25d0*(-xi/(1.d0-ze)-1.d0)
      shp(2,3) = 0.25d0*(xi/(1.d0-ze)+1.d0)
      shp(2,4) = 0.25d0*(-xi/(1.d0-ze)+1.d0)
      shp(2,5) = 0.d0
!
!     local derivatives of the shape functions: mu/zeta-derivative
!
      shp(3,1) = 0.25d0*((et*xi)/((1.d0-ze)*(1.d0-ze))-1.d0)
      shp(3,2) = 0.25d0*(-((et*xi)/((1.d0-ze)*(1.d0-ze)))-1.d0)
      shp(3,3) = 0.25d0*((et*xi)/((1.d0-ze)*(1.d0-ze))-1.d0)
      shp(3,4) = 0.25d0*(-((et*xi)/((1.d0-ze)*(1.d0-ze)))-1.d0)
      shp(3,5) = 1.0d0
!
